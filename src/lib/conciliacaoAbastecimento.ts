...
/* ---------- Abastecimento-specific filters ---------- */

const ALLOWED_SIEG_TAGS = new Set([
  "combustivel externo",
  "combustivel interno",
]);

const ALLOWED_SYSTEM_TYPES = new Set(["nfe", "nfa"]);

/* ---------- Shared helpers (same as NFE) ---------- */
...
function normalizeDate(value: unknown) {
  ...
  if (/^\d+(\.0+)?$/.test(raw)) {
    ...
    if (Number.isFinite(numeric)) return excelDateToISO(numeric);
  }
...
function normalizeCurrency(value: unknown) {
  ...
  const raw = String(value ?? "").trim();
  if (!raw) return 0;
  const sanitized = raw.replace(/\s/g, "").replace(/R\$/gi, "").replace(/\./g, "").replace(",", ".");
...
function isAtivoImobilizado(tagsRaw: string) {
  return splitTags(tagsRaw).map(normalizeTag).some((tag) => tag === "ativo imobilizado");
}
...
/* ---------- Abastecimento-specific tag filter ---------- */

function shouldIncludeAbastecimentoSiegRecord(tagsRaw: string) {
  const normalizedTags = splitTags(tagsRaw).map(normalizeTag);
  return normalizedTags.some((tag) => ALLOWED_SIEG_TAGS.has(tag));
}

function isAllowedSystemType(tipoRaw: string) {
  const normalized = tipoRaw.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
  return ALLOWED_SYSTEM_TYPES.has(normalized);
}

/* ---------- Key builders ---------- */

function getAccessKey(record: ParsedRecord) { return record.normalizedChave; }
function getNFAndCNPJKey(record: ParsedRecord) { return [record.normalizedNumeroNF, record.normalizedCnpjEmitente].join("|"); }
function getCNPJDateAndValueKey(record: ParsedRecord) { return [record.normalizedCnpjEmitente, record.normalizedDataEmissao, record.normalizedValor.toFixed(2)].join("|"); }
function getNFDateAndValueKey(record: ParsedRecord) { return [record.normalizedNumeroNF, record.normalizedDataEmissao, record.normalizedValor.toFixed(2)].join("|"); }
function getNFOnlyKey(record: ParsedRecord) { return record.normalizedNumeroNF; }

/* ---------- Spreadsheet reading ---------- */

function getWorksheet(workbook: XLSX.WorkBook) {
  const firstSheetName = workbook.SheetNames[0];
  if (!firstSheetName) throw new Error("Não foi possível localizar nenhuma aba na planilha enviada.");
  return workbook.Sheets[firstSheetName];
}

function getRowsAsArrays(worksheet: XLSX.WorkSheet) {
  return XLSX.utils.sheet_to_json<unknown[]>(worksheet, { header: 1, defval: "", raw: true });
}
...
function detectHeaderRow(rows: unknown[][], kind: SpreadsheetKind) {
  ...
  rows.slice(0, 20).forEach((row, index) => {
    ...
    const score = targets.reduce((acc, target) => acc + (normalizedCells.some((cell) => cell.includes(target)) ? 1 : 0), 0);
    if (score > bestScore) { bestScore = score; bestIndex = index; }
  });
  if (bestIndex === -1) throw new Error("Não consegui identificar o cabeçalho da planilha.");
  return bestIndex;
}
...
function findColumnIndex(headers: string[], possibleNames: string[]) {
  ...
  for (const name of possibleNames) {
    const normalizedName = normalizeHeader(name);
    const partialIndex = headers.findIndex((header) => header.includes(normalizedName) || normalizedName.includes(header));
    if (partialIndex !== -1) return partialIndex;
  }
  return -1;
}

function getRowValues(row: MappedRow) { return Object.values(row); }

function hasSystemAccessKey(row: MappedRow, chaveIndex: number) {
  if (chaveIndex < 0) return false;
  const values = getRowValues(row);
  return normalizeAccessKey(values[chaveIndex]).length > 0;
}
...
function scoreValueColumn(rows: MappedRow[], columnIndex: number, chaveIndex: number) {
  ...
  for (const row of rows) {
    ...
    checked += 1;
    if (typeof value === "number" && Number.isFinite(value) && value > 0) { score += 4; continue; }
    const raw = String(value).trim();
    if (/[A-Za-z]/.test(raw)) { score -= 3; continue; }
    if (raw.includes("/")) { score -= 3; continue; }
    const normalized = normalizeCurrency(raw);
    if (normalized > 0) { score += 3; } else { score -= 1; }
  }
...
function resolveSystemValueColumnIndex(
  ...
  const blocked = new Set<number>(
    [indexes.tipoIndex, indexes.dataIndex, indexes.nfIndex, indexes.cnpjIndex, indexes.fornecedorIndex, indexes.chaveIndex].filter((index) => index >= 0),
  );
  let bestIndex = -1;
  let bestScore = -9999;
  ...
  if (headerValueIndex !== -1) {
    ...
    if (headerScore >= 8) return headerValueIndex;
    if (bestIndex !== -1 && bestScore > headerScore) return bestIndex;
...
function mapSystemColumns(headers: string[], rows: MappedRow[]): ColumnIndexes {
  ...
  if (missing.length > 0) throw new Error(`Na planilha do sistema não encontrei estas colunas esperadas: ${missing.join(", ")}.`);

  const valorIndex = resolveSystemValueColumnIndex(headers, rows, {
    chaveIndex, nfIndex, dataIndex, cnpjIndex, fornecedorIndex, tipoIndex, statusIndex: -1,
  });

  return { chaveIndex, nfIndex, dataIndex, cnpjIndex, valorIndex, fornecedorIndex, tagsIndex: -1, tipoIndex, statusIndex: -1 };
}
...
function mapGovernmentColumns(headers: string[]): ColumnIndexes {
  ...
  const statusIndex = findColumnIndex(headers, ["Situação", "Situacao", "Status", "Situação da NF", "Situacao da NF", "Sit"]);
  ...
  if (missing.length > 0) throw new Error(`Na planilha do SIEG não encontrei estas colunas esperadas: ${missing.join(", ")}.`);

  return { chaveIndex, nfIndex, dataIndex, cnpjIndex, valorIndex, fornecedorIndex, tagsIndex, tipoIndex: -1, statusIndex };
}
...
function parseSystemRecords(rows: MappedRow[], indexes: ColumnIndexes): ParsedRecord[] {
  return rows
    ...
    .filter((row) => {
      if (indexes.tipoIndex < 0) return true;
      const tipo = String(getRowValues(row)[indexes.tipoIndex] ?? "").trim();
      return isAllowedSystemType(tipo);
    })
    .map((row) => {
      const values = getRowValues(row);
      ...
      const rawDataEmissao = typeof originalData === "number" ? formatDateToBR(normalizeDate(originalData)) : String(originalData ?? "").trim();
  ...
  return rows
    ...
    .map((row) => {
      ...
      return {
        rawChave: String(originalChave ?? "").trim(),
        rawNumeroNF: String(originalNF ?? "").trim(),
        rawDataEmissao: typeof originalData === "number" ? formatDateToBR(normalizeDate(originalData)) : String(originalData ?? "").trim(),
    ...
    .filter(
      (row) =>
        row.normalizedChave || row.normalizedNumeroNF || row.normalizedCnpjEmitente || row.normalizedDataEmissao || row.normalizedValor > 0,
    )
    .filter((row) => shouldIncludeAbastecimentoSiegRecord(row.rawTags));
}
...
export async function parseSpreadsheetFile(file: File, kind: SpreadsheetKind): Promise<ParsedRecord[]> {
  ...
  if (!mappedRows.length) throw new Error("A planilha está vazia ou não possui linhas para leitura.");

  const headers = Object.keys(mappedRows[0] ?? {});
  const indexes = kind === "system" ? mapSystemColumns(headers, mappedRows) : mapGovernmentColumns(headers);
  return kind === "system" ? parseSystemRecords(mappedRows, indexes) : parseGovernmentRecords(mappedRows, indexes);
}

/* ---------- Comparison (identical to NFE) ---------- */
...
function buildComparisonRow(
  govRecord: ParsedRecord, systemRecord: ParsedRecord | null, tipo: DivergenceType, observacao: string, id: string,
): ComparisonRow {
  return {
    id,
...
export function compareReports(governmentRecords: ParsedRecord[], systemRecords: ParsedRecord[]): ComparisonResult {
  ...
  governmentRecords.forEach((govRecord, index) => {
    ...
    if (accessKeyMatches.length > 0) {
      ...
      if (bestMatch.normalizedCnpjEmitente !== govRecord.normalizedCnpjEmitente) divergences.push("cnpj");
      if (bestMatch.normalizedDataEmissao !== govRecord.normalizedDataEmissao) divergences.push("data");
      if (!valuesAreEqual(bestMatch.normalizedValor, govRecord.normalizedValor)) divergences.push("valor");

      if (divergences.length === 0) {
        reconciled += 1;
        results.push(buildComparisonRow(govRecord, bestMatch, "Lançada", "Nota localizada no sistema pela mesma chave de acesso.", id));
        return;
      }
      if (divergences.length === 1) {
        if (divergences[0] === "cnpj") { results.push(buildComparisonRow(govRecord, bestMatch, "CNPJ divergente", "Nota localizada no sistema pela mesma chave de acesso, porém com CNPJ diferente.", id)); return; }
        if (divergences[0] === "data") { results.push(buildComparisonRow(govRecord, bestMatch, "Data divergente", "Nota localizada no sistema pela mesma chave de acesso, porém com data de emissão diferente.", id)); return; }
        results.push(buildComparisonRow(govRecord, bestMatch, "Valor divergente", "Nota localizada no sistema pela mesma chave de acesso, porém com valor diferente.", id));
        return;
      }
      results.push(buildComparisonRow(govRecord, bestMatch, "Múltiplas divergências", "Nota localizada no sistema pela mesma chave de acesso, mas com múltiplas divergências.", id));
      return;
    }
    ...
    if (nfAndCnpjMatches.length > 0) {
      ...
      if (bestMatch.normalizedDataEmissao !== govRecord.normalizedDataEmissao) divergences.push("data");
      if (!valuesAreEqual(bestMatch.normalizedValor, govRecord.normalizedValor)) divergences.push("valor");
      if (divergences.length === 0) { reconciled += 1; results.push(buildComparisonRow(govRecord, bestMatch, "Lançada", "Nota localizada no sistema pelo mesmo número da NF e CNPJ.", id)); return; }
      if (divergences.length === 1) {
        if (divergences[0] === "data") { results.push(buildComparisonRow(govRecord, bestMatch, "Data divergente", "Nota localizada no sistema pelo mesmo número da NF e CNPJ, porém com data de emissão diferente.", id)); return; }
        results.push(buildComparisonRow(govRecord, bestMatch, "Valor divergente", "Nota localizada no sistema pelo mesmo número da NF e CNPJ, porém com valor diferente.", id));
        return;
      }
      results.push(buildComparisonRow(govRecord, bestMatch, "Múltiplas divergências", "Nota localizada no sistema pelo mesmo número da NF e CNPJ, mas com divergência de data e valor.", id));
      return;
    }
    ...
    if (sameNFMatches.length > 0) {
      const cnpjErradoMatch = sameNFMatches.find(
        (item) => item.normalizedCnpjEmitente !== govRecord.normalizedCnpjEmitente && item.normalizedDataEmissao === govRecord.normalizedDataEmissao && valuesAreEqual(item.normalizedValor, govRecord.normalizedValor),
      );
      if (cnpjErradoMatch) { results.push(buildComparisonRow(govRecord, cnpjErradoMatch, "CNPJ errado", "Nota localizada no sistema pelo mesmo número da NF, data de emissão e valor, porém com CNPJ diferente.", id)); return; }

      const bestSameNFMatch = sameNFMatches[0]!;
      const divergences: Array<"cnpj" | "data" | "valor"> = [];
      if (bestSameNFMatch.normalizedCnpjEmitente !== govRecord.normalizedCnpjEmitente) divergences.push("cnpj");
      if (bestSameNFMatch.normalizedDataEmissao !== govRecord.normalizedDataEmissao) divergences.push("data");
      if (!valuesAreEqual(bestSameNFMatch.normalizedValor, govRecord.normalizedValor)) divergences.push("valor");
      if (divergences.length === 1 && divergences[0] === "cnpj") { results.push(buildComparisonRow(govRecord, bestSameNFMatch, "CNPJ divergente", "Nota localizada no sistema pelo mesmo número da NF, porém com CNPJ diferente.", id)); return; }
      if (divergences.length > 0) { results.push(buildComparisonRow(govRecord, bestSameNFMatch, "Múltiplas divergências", "Nota localizada no sistema pelo mesmo número da NF, mas com múltiplas divergências.", id)); return; }
    }
    ...
    if (nfDivergentMatches.length > 0) {
      results.push(buildComparisonRow(govRecord, nfDivergentMatches[0]!, "NF divergente", "Nota localizada no sistema pelo mesmo CNPJ, data de emissão e valor, porém com número da NF diferente.", id));
      return;
    }

    results.push(buildComparisonRow(govRecord, null, "Não lançada", "Nota encontrada no SIEG e não localizada no sistema.", id));
  });

  const notLaunchedRows = results.filter((item) => item.tipo === "Não lançada");
  const divergencesRows = results.filter((item) => item.tipo !== "Não lançada" && item.tipo !== "Lançada");
  ...
  return {
    ...
    summary: {
      ...
      notLaunchedValue: Number(notLaunchedRows.reduce((acc, item) => acc + item.valor, 0).toFixed(2)),
      divergencesValue: Number(divergencesRows.reduce((acc, item) => acc + item.valor, 0).toFixed(2)),
    },
  };
}

export function exportFilteredToExcel(rows: ComparisonRow[], filterLabel: string) {
  const rowsToExport = normalizeTag(filterLabel) === "ativo imobilizado" ? rows.filter((item) => item.ativoImobilizado) : rows;
  ...
  if (!mapped.length) throw new Error("Nenhum registro encontrado para exportação.");

  const worksheet = XLSX.utils.json_to_sheet(mapped);
  const workbook = XLSX.utils.book_new();
  worksheet["!cols"] = [{ wch: 48 }, { wch: 16 }, { wch: 16 }, { wch: 22 }, { wch: 32 }, { wch: 28 }, { wch: 18 }, { wch: 14 }, { wch: 14 }, { wch: 22 }, { wch: 70 }];

  const safeSheetName = normalizeTag(filterLabel) === "ativo imobilizado" ? "Ativo Imobilizado" : filterLabel.slice(0, 31) || "Abastecimento";
  XLSX.utils.book_append_sheet(workbook, worksheet, safeSheetName);
  XLSX.writeFile(workbook, `conciliacao-abastecimento-${safeSheetName.toLowerCase().replace(/\s+/g, "-")}.xlsx`);
}
