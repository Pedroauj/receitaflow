  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="rounded-lg border border-success/25 bg-success/10 p-4 text-center">
          <p className="text-xs text-muted-foreground mb-1">Ganho Total</p>
          <p className="text-lg font-bold text-success-foreground">+{ganho.toFixed(0)} L</p>
          <p className="text-xs text-success-foreground/70 mt-0.5">{formatBRL(valorGanho)}</p>
        </div>
        <div className="rounded-lg border border-destructive/25 bg-destructive/10 p-4 text-center">
          ...
          <p className="text-lg font-bold text-destructive">-{perda.toFixed(0)} L</p>
          <p className="text-xs text-destructive/70 mt-0.5">{formatBRL(valorPerda)}</p>
        </div>
        <div className={`rounded-lg border p-4 text-center ${saldo >= 0 ? "border-success/25 bg-success/10" : "border-destructive/25 bg-destructive/10"}`}>
          <p className="text-xs text-muted-foreground mb-1">Saldo</p>
          <p className={`text-lg font-bold ${saldo >= 0 ? "text-success-foreground" : "text-destructive"}`}>
      ...
      {topWaste.length > 0 && (
        <>
          ...
          <div className="space-y-2">
            {topWaste.map(r => (
              <div key={r.id} className="flex items-center justify-between rounded-lg border border-destructive/25 bg-destructive/10 px-4 py-2.5">
                <div>
                ...
                <span className="text-sm font-bold text-destructive">{r.ganhoPerda.toFixed(0)} L</span>
              </div>
            ))}
          </div>
        </>
